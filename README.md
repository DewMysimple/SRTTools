...
![SRTTools 合并后的文本转换工作台](docs/images/workspace.png)

## 选对功能

| 你的需求 | 工作台 / 处理方式 | 得到什么 |
| --- | --- | --- |
| SRT 改成 TXT，内容一点不变 | 文本转换 → 原样转 TXT | 保留原始字节、编码、换行、序号和时间轴的 TXT |
| SRT 只要字幕正文 | 文本转换 → SRT 提取正文 | 去掉序号与时间轴，保留多行、中文和双语内容 |
| 已有 TXT 里还残留 SRT 信息 | 文本转换 → TXT 字幕清理 | 删除可识别的时间轴及其结构序号，保留正文数字 |
...
1. 左侧选择「文本转换」或「时间范围导出」；文本转换在顶部工具栏选择处理方式，添加或拖入文件。
...
三种文本处理共用一个工作台，切换时保留文件列表、输出位置和正文设置，重新预览后即可导出。原样模式禁用输出编码、排版与去标签，保证原始字节不变。「重置选项」恢复当前方式的默认设置，不清空文件或输出位置。
...
├── srttools/
│   ├── app.py / ui.py    # 两个工作台、文本处理工具栏与受控后台线程
