...
ALLOWED_STATUSES = {"active", "proposed", "deprecated", "superseded", "archived", "draft"}
ALLOWED_KINDS = {
    "feature",
    "ui",
    "bug",
...
REQUIRED_FIELDS = {"type", "status", "kind", "importance", "updated", "topic", "source_logs", "supersedes"}
...
def resolve_link(source: Path, target: str) -> Path | None:
    ...
    if not candidate.suffix and not candidate.name.startswith("."):
...
def page_links(page: Page) -> Iterable[str]:
    yield from links_in(page.body)
    sources = page.fields.get("source_logs", [])
    for value in sources if isinstance(sources, list) else []:
        yield from links_in(str(value))
    if page.fields.get("supersedes"):
        yield from links_in(str(page.fields["supersedes"]))


def normalized_link(root: Path, source: Path, target: str) -> str | None:
    resolved = resolve_link(source, target)
    if resolved is None:
        return None
    # Resolve actual ../ paths, preserving links to code outside wiki_memory.
    return Path(os.path.relpath((root / resolved).resolve(), root.resolve())).as_posix()
...
def validate_pages(root: Path, pages: list[Page]) -> list[str]:
    ...
    for page in pages:
        ...
        if is_managed(page.path):
            if page.fields.get("importance") not in {"high", "medium", "low"}:
                errors.append(f"{page.rel}: invalid importance")
            try:
                date.fromisoformat(str(page.fields.get("updated", "")))
            except ValueError:
                errors.append(f"{page.rel}: updated must be a real YYYY-MM-DD date")
            if not isinstance(page.fields.get("source_logs"), list):
                errors.append(f"{page.rel}: source_logs must be a list")

        for target in page_links(page):
            normalized = normalized_link(root, page.path, target)
...
def index_logs(root: Path, pages: list[Page]) -> Path:
    ...
    lines = [
        ...
        "kind: process",
        "importance: high",
        f"updated: {max((str(page.fields.get('updated', '')) for page in logs), default=date.today().isoformat())}",
    ...
    entry_links = [
        (Path("README.md"), "[SRTTools 工程记忆](../README.md)"),
        (Path("AGENTS.md"), "[记忆维护协议](../AGENTS.md)"),
        (Path("日志/README.md"), "[工作日志说明](./README.md)"),
        (Path("当前状态/项目概览.md"), "[当前项目概览](../当前状态/项目概览.md)"),
        (Path("当前状态/系统架构.md"), "[当前系统架构](../当前状态/系统架构.md)"),
    ]
